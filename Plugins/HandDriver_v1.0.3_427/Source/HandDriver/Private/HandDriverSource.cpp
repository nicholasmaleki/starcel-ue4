...
#define LOCTEXT_NAMESPACE "HandDriverSource"

#define RECV_BUFFER_SIZE 1024 * 1024

FHandDriverSource::FHandDriverSource(FIPv4Endpoint Endpoint)
	:Socket(nullptr)
	, Thread(nullptr)
	, Stopping(false)
	, WaitTime(FTimespan::FromMilliseconds(100))
{
	...
	SourceType = LOCTEXT("HandDriverSourceType", "HandDriver LiveLink");
	SourceMachineName = LOCTEXT("HandDriverSourceMachineName", "localhost");

	//setup socket
	if (DeviceEndpoint.Address.IsMulticastAddress())
	{
		Socket = FUdpSocketBuilder(TEXT("HANDSOCKET"))
			.AsNonBlocking()
			.AsReusable()
...
FHandDriverSource::~FHandDriverSource()
{
	Stop();
	if (Thread != nullptr)
	{
...
void FHandDriverSource::ReceiveClient(ILiveLinkClient* InClient, FGuid InSourceGuid)
{
	Client = InClient;
	SourceGuid = InSourceGuid;
}

bool FHandDriverSource::IsSourceStillValid() const
{
	bool bIsSourceValid = !Stopping && Thread != nullptr && Socket != nullptr;
	return bIsSourceValid;
}

bool FHandDriverSource::RequestSourceShutdown()
{
	Stop();
	return true;
}

void FHandDriverSource::Start()
{
	ThreadName = "HAND UDP Receiver ";
	ThreadName.AppendInt(FAsyncThreadIndex::GetNext());
	Thread = FRunnableThread::Create(this, *ThreadName, 128 * 1024, TPri_AboveNormal, FPlatformAffinity::GetPoolThreadMask());
}

void FHandDriverSource::Stop()
{
	Stopping = true;
}

uint32 FHandDriverSource::Run()
{
...
void FHandDriverSource::HandleReceivedData(TSharedPtr<TArray<uint8>, ESPMode::ThreadSafe> ReceivedData) 
{
	if (Stopping)
	{
		return;
