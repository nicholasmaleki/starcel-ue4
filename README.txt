...
## Quickstart
The current version offers the full Unreal Engine source project, ~35GB, and therefore is quite a large download. You can download individual files directly from github if needed. Future releases will separately include the client, the server, and the editor. 

Compiled versions of Python 3.9 can be downlaoded from here https://www.python.org/downloads/release/python-3913/, newer versions of Python 3.9 should work, but need to be compiled from Python source. The unreal_engine package is not installed to Python permanently, but it is installed at runtime. The Content/Scripts/unreal_engine folder includes all necessary stubs and empty classes for intellisense. 
You can install the necessary packages directly using python.exe -m pip install sympy cmdix fast-autocomplete fast-autocomplete[levenshtein] numba kingdon 
The default Unreal Engine map contains a PyActor which loads main.py and the class Main when the map starts up. The line ``if KismetSystemLibrary.IsDedicatedServer():`` lets you define client and server specific code. You can create more PyActors or even use python from the UE console by clicking `. 
Create a Text object by double clicking Enter. Once created, you can use =py() to run any Python command. 
The state of the world is not saved in main.py or elsewhere, you will need to make changes there directly or copy your changes from the world elsewhere to save. 
...
Python support can be found at r/LearnPython and other forums and chats.
Unreal Engine support can be found on their Discord and their forum. 
UnrealEnginePython examples, Starcel examples, and the unreal_engine folder in Content/Scripts which has the class and function info should cover just about everything you can do with the plugin. If you are certain there is an issue with the 20tab/UnrealEnginePython plugin you can post on their issues tab on GitHub. I will check this every so often. The UE4.27 port of the plugin can be found at https://github.com/HaiyiMei/UnrealEnginePython. I made minor changes to this source to make it work on my computer. Issues with the Starcel code and new features can be posted here. I will occasionally merge new features and fixes. Feel free to contribute. Anything else can be asked in our Discord. We are a community of volunteers so we may not have answers and may not get back to you, but we will try our best. 
...
Hotkey editor
Multi-language IDE
Symbols with varying sizes replacement, highlighting, and reduction: code translation. Rotating symbol replacement so you don't get used to random letters and symbols. code_report/jello.
File explorer and desktop environment.
CLI, ssh, and scp. 
Math, algebra, calculus, latex. API for the Reals.
Geometric algebra rendering: PGA, VGA, CGA, STA
Full Excel and Google Sheets importer
Plotting, charts, digraphs, nodes. Most of what is available in OriginLab Pro. 
VRChat/Helios features
Multidimensional graphs and 
...
Academic hall and learning center: history, particles/physical matter, math, physics. 
