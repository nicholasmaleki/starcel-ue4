...
import numpy as np
from nd_table_v2 import Table, Axis, create_cell_globals
from spread_engine_advanced import SpreadEngine, SpreadVisualizer
from pga_integration import PGATable, PGAPrimitives, PGAConverter, KINGDON_AVAILABLE
...
def example_variable_syntax():
    ...
    t = Table(shape=(5, 5), debug=False, aggressive_debug=False)
...
def example_undo_redo():
    ...
    t = Table(shape=(3, 3), enable_undo=True, aggressive_debug=True)
...
def example_cross_table():
    ...
    # Create two tables
    prices = Table(shape=(3, 3), aggressive_debug=False)
    prices['A1'] = 100
    prices['A2'] = 200
    prices['A3'] = 300
    
    quantities = Table(shape=(3, 3), aggressive_debug=False)
    quantities['A1'] = 10
    quantities['A2'] = 20
    quantities['A3'] = 30
    ...
    totals = Table(shape=(3, 3), aggressive_debug=True)
...
def example_enhanced_axes():
    ...
    t = Table(axes=[axis1, axis2], aggressive_debug=False)
...
def example_always_reevaluate():
    ...
    t2 = Table(shape=(3, 3), lazy_eval=True, always_reevaluate=True, aggressive_debug=True)
    t2['A1'] = 10
    t2['A2'] = "=A1 * 2"
...
def example_aggressive_debug():
    ...
    t = Table(shape=(3, 3), aggressive_debug=True)
...
def example_pga_integration():
    ...
    # Create PGA table
    pga_table = PGATable(shape=(5, 5), pga_signature=(3, 0, 1), debug=True)
